/* ==========================================================================
   ...
   - REALTIME wallet + vote updates via Server-Sent Events (SSE)
========================================================================== */
...
function startRealtime(){
  if (__sse) { try { __sse.close(); } catch {} }
  try {
    __sse = new EventSource('/api/admin/stream', { withCredentials: true });
    const chip = document.getElementById('liveChip');
    __sse.addEventListener('hello', () => { chip && chip.classList.add('connected'); });

    __sse.addEventListener('wallet', ev => {
      try {
        const d = JSON.parse(ev.data);
        const bal = 'KES ' + (d.balance||0).toLocaleString();
        const wEl = document.getElementById('s-wallet');   if (wEl) { wEl.textContent = bal; pulse(wEl); }
        const bEl = document.getElementById('walletBig');  if (bEl) { bEl.textContent = bal; pulse(bEl); }
        const sEl = document.getElementById('walletSuccess'); if (sEl) sEl.textContent = d.successfulVotes || 0;
        const scEl = document.getElementById('s-success'); if (scEl) scEl.textContent = d.successfulVotes || 0;
      } catch {}
    });

    __sse.addEventListener('vote', ev => {
      try {
        const d = JSON.parse(ev.data);
        prependFeed('realtimeFeed', d);
        if (d.status === 'SUCCESS') prependFeed('walletActivity', d);
        toast(d.status==='SUCCESS'
          ? `✓ ${d.nomineeName} received ${d.amount} votes (KES ${d.amount})`
          : `✗ Failed payment for ${d.nomineeName}`,
          d.status==='SUCCESS' ? 'success' : 'error');
      } catch {}
    });

    __sse.addEventListener('nominee', ev => {
      try {
        const d = JSON.parse(ev.data);
        if (d.type === 'created') {
          toast(`New nominee registered: ${d.nominee.name}`, 'info');
          const cur = document.querySelector('.menu-item.active');
          if (cur && cur.dataset.page === 'nominees') loadNominees();
          if (cur && cur.dataset.page === 'dashboard') loadDashboard();
        }
      } catch {}
    });
    ...
    __sse.onerror = () => {
      const chip = document.getElementById('liveChip');
      chip && chip.classList.remove('connected');
...
function prependFeed(elId, d){
  const el = document.getElementById(elId); if (!el) return;
  const item = document.createElement('div');
  item.className = 'feed-item ' + (d.status === 'SUCCESS' ? 'ok' : 'fail');
  const icon = d.status === 'SUCCESS' ? 'check-circle' : 'times-circle';
  item.innerHTML = `
    <i class="fas fa-${icon}"></i>
    <div class="feed-body">
      <div><b>${escapeHtml(d.nomineeName||'-')}</b> · <span>KES ${d.amount||0}</span></div>
      <div class="muted"><small>${d.status}${d.transactionId?' · '+escapeHtml(d.transactionId):''}${d.phone?' · '+escapeHtml(d.phone):''}</small></div>
    </div>
    <div class="feed-time muted"><small>${new Date().toLocaleTimeString()}</small></div>`;
  // Clear placeholder text on first item
  if (el.querySelector('p.muted')) el.innerHTML = '';
  el.prepend(item);
...
const pageTitles = {
  ...
  wallet:       ['Wallet',       'Realtime financial balance'],
...
async function loadWallet(){
  try {
    ...
    // Populate wallet activity from recent successful votes
    const votes = await api('/api/admin/votes');
    const success = votes.filter(v => v.status === 'SUCCESS').slice(0, 10);
    const feed = document.getElementById('walletActivity');
    if (!success.length) { feed.innerHTML = '<p class="muted">Recent successful transactions will appear here...</p>'; }
    else feed.innerHTML = success.map(v => `
      <div class="feed-item ok">
        <i class="fas fa-check-circle"></i>
        <div class="feed-body">
          <div><b>${escapeHtml(v.nomineeName||'-')}</b> · <span>KES ${v.amount}</span></div>
          <div class="muted"><small>${escapeHtml(v.transactionId||v.checkoutRequestID||'-')} · ${escapeHtml(v.phone||'-')}</small></div>
        </div>
        <div class="feed-time muted"><small>${new Date(v.completedAt||v.createdAt).toLocaleString()}</small></div>
      </div>`).join('');
  } catch(e){ toast(e.message,'error'); }
}
...
async function loadSettings(){
  try {
    ...
    // Show current callback URL - built from current origin
    const cb = document.getElementById('cbUrl');
    if (cb) cb.textContent = location.origin + '/callback';
  } catch(e){ toast(e.message,'error'); }
}
